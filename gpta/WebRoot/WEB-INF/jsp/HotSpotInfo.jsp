...
<!--                   </li> -->
                ...
                <div class="box-body">
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <c:forEach var="hotspot" items="${hotspot}">
                      <tr>
                      	<td>${hotspot.title }</td>
                        <td>${hotspot.spotname }</td>
                        <td>${hotspot.date }</td>
                        <td>
                        <c:if test="${hotspot.discount==0 }">${hotspot.price }/${hotspot.price}</c:if>
                        <c:if test="${hotspot.discount!=0 }">${hotspot.price }/<fmt:formatNumber value="${hotspot.price * (0.01*hotspot.discount) }" pattern="0.00" maxFractionDigits="1"/>
                        
                        </c:if></td>
                        <td>${hotspot.discount }%</td>
                        <td>${hotspot.remark }</td>
                        <td>
                        <a href="hotspot.do?m=updatehotspot&id=${hotspot.id }">修改</a>|
                        <a href="hotspot.do?m=delhotspotbyid&id=${hotspot.id }" onClick="return confirm('确认删除?');">删除</a>
                        </td>
                        <td><a href="order.do?m=getorderbyspotid&spotid=${hotspot.id }">详情</a></td>
                      </tr>
                     </c:forEach>
                    </tbody>
	...
	function detail(id){
		$.ajax({
  				type: 'post',
  				url: "order.do?m=getorderbyspotid",
  				data:{
  					spotid:id
  				},
